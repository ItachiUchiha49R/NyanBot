//============ IshigamiBot ============\\
//   creditos: Ishigami,Elissa Y Shido Senpai
//============ Ishigami ============\\
const { 
  WAConnection,
  MessageType,
...
const ishigamiGg = require('google-it');
const ishigamiGgImg = require('g-i-s');
////////////▶ 𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 | 𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 𝐘 𝐄𝐥𝐢𝐬𝐬𝐚
...
const ishigami = JSON.parse(fs.readFileSync('./setting.json'))
const bodyM = ishigami.ishigamiM
...
Ishigami.on('CB:action,,call', async json => {
...
Ishigami.sendMessage(callerId, "Las llamadas no se permiten, *PORFAVOR LEE LAS REGLAS!* Te bloqueare😒", MessageType.text, {quoted: {key: {
fromMe: false,
...
await sleep(4000)
await Ishigami.blockUser(callerId, "add")
})
////////////▶ 𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 | 𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 𝐘 𝐄𝐥𝐢𝐬𝐬𝐚
Ishigami.on('CB:action,,battery', json => {
...
////////////▶ 𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 | 𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 𝐘 𝐄𝐥𝐢𝐬𝐬𝐚
Ishigami.on('blocklist-update', async (chat) => {
for (i of chat.added){
...
////////////▶ 𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 | 𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 𝐘 𝐄𝐥𝐢𝐬𝐬𝐚
Ishigami.on('group-update', async(chat) => {
var donde = chat.jid
var group = await Ishigami.groupMetadata(donde)
if (!chat.desc == '') {
...
Ishigami.sendMessage(group.id, mensajeDesc, MessageType.text)
...
try {
pushnem = sam.key.fromMe ? Ishigami.user.name : conts.notify || conts.vname || conts.name || '-'

} catch {
pushnem = num.split('@')[0]
}
try {
ppimg = await Ishigami.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
} catch {
...
Ishigami.sendMessage(mdata.id, fs.readFileSync('./hamsil.jpg'), MessageType.image, {quoted: {key: {
fromMe: false,
participant: `0@s.whatsapp.net`          
},                               
message: {                         
...
Ishigami.sendMessage(mdata.id, `😙Hola, @${num.split('@')[0]}, _*Bienvenido a ${mdata.subject}, esperamos que te la pases a gusto en este grupo✨*_\n\n_Recuerda siempre seguir las reglas y mantener una formalidad respetuosa_😉\n\nSon las *${jm}* del *${calender}*\n\n${mdata.desc}`, MessageType.text)
}
//leave
...
Ishigami.sendMessage(mdata.id, teks, MessageType.text,{ contextInfo: {"mentionedJid": [num]}})

} else if (anu.action == 'promote') {
num = anu.participants[0]
try {
ppimg = await Ishigami.getProfilePicture(`${num.split('@')[0]}@c.us`)
} catch {
ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
}
thu = await Ishigami.getStatus(anu.participants[0], MessageType.text)
teks = `*✅NUEVO ADMIN✅*\n*🙋🏻‍♂️ Nombre*: @${num.split('@')[0]}\n*📋 INFO*: ${thu.status}\n\n🥳 *FEILICIDADES!!*, te as convertido en administrador del grupo ${mdata.subject}`
let buff = await getBuffer(ppimg)
Ishigami.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
...
////////////▶ 𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 | 𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 𝐘 𝐄𝐥𝐢𝐬𝐬𝐚
Ishigami.on('chat-update', async(sam) => {
    try {
	...
	const crypto = require('crypto')
        ...
        const isCmd = cmd
        const meNumber = Ishigami.user.jidi
        const botNumber = Ishigami.user.jid.split("@")[0]
        const isGroup = from.endsWith('@g.us')
	const sender = sam.key.fromMe ? Ishigami.user.jid : isGroup ? sam.participant : sam.key.remoteJid
        const senderNumber = sender.split("@")[0]
        const groupMetadata = isGroup ? await Ishigami.groupMetadata(from) : ''
        const groupName = isGroup ? groupMetadata.subject : ''
        const groupMembers = isGroup ? groupMetadata.participants : ''
        const groupAdmins = isGroup ? await wa.getGroupAdmins(groupMembers) : []
        const isAdmin = groupAdmins.includes(sender) || false
        const botAdmin = groupAdmins.includes(Ishigami.user.jid)
	...
	const totalchat = await Ishigami.chats.all()
	...
	conts = sam.key.fromMe ? Ishigami.user.jid : Ishigami.contacts[sender] || {
                notify: jid.replace(/@.+/, '')
	}
	const jid = sender
	Ishigami.chatRead(from)
	const Smname = sam.key.fromMe ? Ishigami.user.jid : Ishigami.contacts[sender] || { notify: jid.replace(/@.+/, '') }
	...
	(id == null || id == undefined || id == false) ? Ishigami.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : Ishigami.sendMessage(from, teks.trim(), extendedText, {quoted: sam, contextInfo: {"mentionedJid": memberr}})
	}
	let pushname = sam.key.fromMe ? Ishigami.user.name : conts.notify || conts.vname || conts.name || '*Amigo*'
	const isUrl = (url) => {
	...
	Ishigami.sendMessage(hehe, teks, MessageType.text, {quoted: ftoko})
  	}
	...
	mess = {
			...
			only: {
    			...
    			ownerB: '[❗] ¡Este comando solo puede ser utilizado por el creador del bot! ❌\nOsea, ishigami: wa.me/+529984907794, Habla con el para que pueda cambiar el numero del owner en este bot',
	...
	const sendFile = async (archivo, nombreDeArchivo, comentario, tag, vn) => {
  	...
  	Ishigami.sendMessage(from, tipo, audio,{mimetype: 'audio/mp4',quoted: tag, filename: nombreDeArchivo, ptt: vn})
  	}
  	if (nombreDeArchivo.includes('mp4')){
	Ishigami.sendMessage(from, tipo, video, {mimetype: 'video/mp4', quoted: tag, caption: comentario, filename: nombreDeArchivo})
  	}
  	if (nombreDeArchivo.includes('gif')){
  	Ishigami.sendMessage(from, tipo, video, {mimetype: Mimetype.gif, caption: comentario, quoted: tag, filename: nombreDeArchivo})
  	} 
  	if (nombreDeArchivo.includes('png')){
  	Ishigami.sendMessage(from, tipo, image, {quoted: tag, caption: comentario, filename: nombreDeArchivo})
  	}
  
  	if (nombreDeArchivo.includes('webp')){
  	Ishigami.sendMessage(from, tipo, sticker, {quoted: tag})
  	} else {
  	tipo2 = nombreDeArchivo.split(`.`)[1]
  	Ishigami.sendMessage(from, tipo, document, {mimetype: tipo2, quoted: tag, filename: nombreDeArchivo})
  	}
	}
	    
	const sendFileFromUrl = async(link, type, options) => {
  	hasil = await getBuffer(link)
	Ishigami.sendMessage(from, hasil, type, options).catch(e => {
	fetch(link).then((hasil) => {
	Ishigami.sendMessage(from, hasil, type, options).catch(e => {
	Ishigami.sendMessage(from, { url : link }, type, options).catch(e => {
	...
	┎┈┈┈┈┈┈┈┈┈┈┈┈┈┈
  	...
  	🕋rango: ${nivelActual}
	┖┈┈┈┈┈┈┈┈┈┈┈┈┈┈`
	/*Ishigami.sendMessage(from, lvup, MessageType.text, {quoted: { key: {                
		fromMe: false,
	...
	const reply = async(teks) => {
                await Ishigami.sendMessage(from, teks, MessageType.text, { quoted: { key: {                
		fromMe: false,
                participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
                },
                message: {
	...
	const fspam = {
		...
		"vcard": 'BEGIN:VCARD\n' +
    		'Version:3.0\n' +
    		'TEL;type=CELL;type=VOICE;waid=522971046999:+522971046999\n' +
...
mentionedJid: [sender]}
		...
		if (messagesC.includes("bot")){
			Ishigami.updatePresence(from, Presence.composing)
			rm = [
			result = fs.readFileSync(`./temp/ishigami.webp`),
			result1 = fs.readFileSync(`./temp/ishigami1.webp`),
			result2 = fs.readFileSync(`./temp/ishigami2.webp`),
			result3 = fs.readFileSync(`./temp/ishigami3.webp`),
			result4 = fs.readFileSync(`./temp/ishigami4.webp`)
			]
			nk = rm[Math.floor(Math.random() * rm.length)]
  			Ishigami.sendMessage(from, nk, sticker, {
			...
			var kic = `${sender.split("@")[0]}@s.whatsapp.net`
                        ...
                        setTimeout( () => {
                                Ishigami.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
                        }, 5000)
                        setTimeout( () => {
                                Ishigami.updatePresence(from, Presence.composing)
                                reply("1 segundo")
		...
		.save(`./sticker/${sender}.webp`)
			...
			Ishigami.groupRemove(from, [sender])
		}

	    
	    ///////////////////////FUNCIONES CREADAS POR Ishigami\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
			...
			if (!itsMe) {
                      	if (q.length > 10000) {
				reply('*Este mensaje contiene mas de 10, 000 caracteres, probablemente puede ser una trava, por lo que tendre que eliminarte🙂*\n\n_Este grupo esta protegido por_ *🔐Ishigami*')
				Ishigami.groupSettingChange(from, GroupSettingChange.messageSend, true).then(() => {
				Ishigami.sendMessage(from, '*Esperemos 10 segundos🙄*', MessageType.text)
				})
				Ishigami.groupRemove(from, [sender])
				await sleep(10000)
				Ishigami.groupSettingChange(from, GroupSettingChange.messageSend, false)
			}
			}
			}
			if (isGroup && botAdmin && isAntiLeg) {                                                         	  
...
redes = ['*Sigeme y te sigo en instagram!* https://www.instagram.com/1ishigami/']
...
const forder = { key : {fromMe: false,participant : "0@s.whatsapp.net", ...(from ? { remoteJid: "5214447000377-1624232428@g.us" } : {})},message: {orderMessage: {itemCount : 999999999,status: 1,surface : 1,message: `🥀𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 | 𝐼𝑠𝒉𝑖𝑔𝑎𝑚𝑖𝐵𝑜𝑡🍒`,orderTitle: 'Ishigami',sellerJid: `🌱`,thumbnail: fs.readFileSync('./src/fake.jpg')}}}
Ishigami.updatePresence(from, Presence.recording)
uptime = process.uptime()
		
if (!isRegister) return Ishigami.sendMessage(from, assistant, image, { quoted: noreg, caption: `😊Hola, ${timeFt}.\n*Yo soy Ishigami*, Asistente de *Ishigami*!.\n\nAl parecer no estas registrado en _*IshigamiBot*_, Para registrarte usa el comando: *${prefix}reg*.`, thumbnail: assistant, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
try {		
Menu = `
➫ြ𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 𝐘 𝐄𝐥𝐢𝐬𝐬𝐚.li
...
${ishigami} ✏Prefijo:${ishigami} [ ${prefix} ]
${ishigami} 🕐Tiempo de actividad:${ishigami} *${uptime}*
${ishigami} ✅Modo:${ishigami} *ON*
${ishigami} 👥Grupo:${ishigami} *${groupName}*
${ishigami} 🏆Numero del Dueño wa.link/wpnz32${ishigami}

𝗠𝗬 𝗖𝗔𝗡𝗔𝗟 𝗗𝗘 𝗬𝗢𝗨𝗧𝗨.𝗕𝗘: shrtco.de/CanalDeishigami

⍣ *BOT INFO* ⍣
${ishigami}◦ 🌐Navegador :${ishigami} *${Ishigami.browserDescription[1]}*
${ishigami}◦ 📡servidor :${ishigami} *${Ishigami.browserDescription[0]}*
${ishigami}◦ ✅version :${ishigami} *${Ishigami.browserDescription[2]}*
${ishigami}◦ 🚄Velocidad :${ishigami} *${process.uptime()}*
${ishigami}◦ 📲Sistema operativo :${ishigami} *${Ishigami.user.phone.device_manufacturer}*
${ishigami}◦ 🪀version de${ishigami} *WhatsApp* : *${Ishigami.user.phone.wa_version}*
...
╰──────────────╯`
Ishigami.sendMessage(from, stc, MessageType.text, {quoted:
{ key: {
fromMe: false,
participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
},
...
│ြ𝚜ᷤ𝚊ͣ𝚖ͫ𝚞𝉄𖾔𖾔𖽙😈.li Oℱịcιɑl.li🌴
╰───────────────╯`
Ishigami.sendMessage(from, mda, MessageType.text, {quoted:
{ key: {
fromMe: false,
participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
},
...
const Menug = {
text: `➫ြ𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 𝐘 𝐄𝐥𝐢𝐬𝐬𝐚.li                                                                            
🔐Hola *${pushname}*

${bodyM} $ishigami}${prefix}antilink${ishigami}
${bodyM} ${ishigami}${prefix}antimedia${ishigami}
${bodyM} ${ishigami}${prefix}antibad${ishigami}
${bodyM} ${ishigami}${prefix}autostick${ishigami}
${bodyM} ${ishigami}${prefix}antileg${ishigami}

${bodyM} ${ishigami}${prefix}antigp${ishigami}
...
Ishigami.sendMessage(from, Menud, MessageType.text, {
quoted:  fvid})
addFilter(from)
addLevelingLevel(sender, 5)		
break
...
Ishigami.sendMessage(from, Menuo, MessageType.text, {
quoted: floc})
addFilter(from)
addLevelingLevel(sender, 5)		
break
...
ishigamiPn = fs.readFileSync('./src/+18.jpg')
uptime = process.uptime()
const Menu18 = `➫ြ𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 𝐘 𝐄𝐥𝐢𝐬𝐬𝐚.li
...
addFilter(from)
addLevelingLevel(sender, 5)		
Ishigami.sendMessage(from, ishigamiPn, image, { quoted: fvid, caption: `${Menu18}`, thumbnail: ishigamiPn, contextInfo: { mentionedJid: [sender], "forwardingScore": 9999, "isForwarded": true }})              
break
case 'menu7':
Ishigami.updatePresence(from, Presence.composing)
if (!isRegister) return reply(mess.only.usrReg)
...
Ishigami.sendMessage(from, Menu7, MessageType.text, {
quoted: fvid})
addFilter(from)
addLevelingLevel(sender, 5)		
break
...
Ishigami.sendMessage(from, Menu8, MessageType.text, {
quoted: ftoko})
addFilter(from)
addLevelingLevel(sender, 5)		
break
...
💠Si quieres ser inmune a estos comandos, ishigami a creado un WhatsApp que soporta estos bugs, si quieres probar este WhatsApp comunicate con el:
...
Ishigami.sendMessage(from, smww, image, {caption: `${menu9}`, quoted: { key: { 
	fromMe: false, 
	participant: `0@s.whatsapp.net`, ...(from ? {
		remoteJid: "status@broadcast" } : {}) }, 
			message: { 
...
Ishigami.sendMessage(from, aud, audio, {quoted: faud, mimetype: 'audio/mp4', ptt: true, duration: -999999, sendEphemeral: true}) 
break
case 'anana':
addFilter(from)
...
case 'p2':
nopsw = Ishigami.prepareMessageFromContent(from,{ "listMessage": {
	...
	"sections": [{ "rows": [ { "title": `a`, 
		...
		Ishigami.relayWAMessage(nopsw)
break
		
case 'b1':
b1 = Ishigami.prepareMessageFromContent(from,{ "buttonsMessage": { 
  ...
  {buttonId: 'id2', buttonText: {displayText: 'Button 2'}, type: 1}
	...
	headerType: 1
		...
		Ishigami.relayWAMessage(b1)
		Ishigami.sendMessage(from, b1, MessageType.buttonsMessage)
		break
...
let search = await ishigamiGg({ query: buscar })
let ggsm = ``
for (let i of search) {
ggsm += `
*Titulo :* ${i.title}
...
reply(`*🔍Busqueda realizada por* 🐉Ishigami🐉\n\n${nyangg}`)
addFilter(from)
break
...
case 'update':
if (!itsMe) return reply('tu quien eres para decirme que hacer!?🤔')
reply('*ESPERE UN MOMENTO... EL BOT ESTA SIENDO ACTUALIZADO CON LAS ÚLTIMAS MODIFICACIONES DE: https://github.com/Ishigami/IshigamiBot*')
...
case 'grupos':
Ishigami.updatePresence(from, Presence.composing)  
let gruposT = Ishigami.chats.array.filter(v => v.jid.endsWith('g.us')).map(v =>`${Ishigami.groupMetadata(v.jid).subject}\n${v.jid} [${v.read_only ? ' ❌Salio ' : ' *✅Dentro* '}]`).join`\n\n`
Ishigami.sendMessage(from, `*Lista de Grupos del Bot:*\n\n${gruposT}`, MessageType.text, {quoted: floc})
break
...
if (args.length >= 300) return Ishigami.sendMessage(from, '*El limite del reporte es de maximo 300 caracteres!*', MessageType.text, {quoted: ftoko})
var numerorepo = sam.participant
reporte = `[REPORTE]\nDe: @${sender.split("@s.whatsapp.net")[0]}\n\n${q}`
var options = { text: reporte, contextInfo: { mentionedJid: [sender] },}
Ishigami.sendMessage('522971046999@s.whatsapp.net', options, MessageType.text, {quoted: floc})
...
await Ishigami.sendMessage(from, ini_buffer, sticker)
}
break
...
sendFileFromUrl(waifu.url, image, {quoted: fimg, caption: '💎 *Ishigami | IshigamiBot* 💠', sendEphemeral: true})
addFilter(from)
break
...
var { id, owner, subject, subjectOwner, desc, descId, participants, size, descOwner, descTime, creation} = await Ishigami.query({ 
json: ["query", "invite", net],
expect200:true })
let insSm = `_*Inspección By Ishigami💎*_
...
Ishigami.sendMessage(from, insSm, MessageType.text, {quoted: fliveLoc})
break
...
const dlfile = await Ishigami.downloadMediaMessage(encmediats)
reply(mess.wait)
const bas64 = `data:image/jpeg;base64,${dlfile.toString('base64')}`
var mantap = await convertSticker(bas64, `${author2}`, `${pack}`)
var imageBuffer = new Buffer.from(mantap, 'base64');
Ishigami.sendMessage(from, imageBuffer, sticker, {quoted: sam})
addFilter(from)
break
...
const dlfile1 = await Ishigami.downloadMediaMessage(encmedia1)
...
Ishigami.sendMessage(from, st, sticker, {quoted: sam})
...
contextInfo : sam
const media2 = await Ishigami.downloadAndSaveMediaMessage(encmedia2, `./sticker/${sender}`)
const packname101 = `\n\n\n\n\n\n\n\n\n\n\nIshigami IshigamiBot\n\n       Ishigami y Eliissa`
const author101 = args.join(' ')
...
${ishigami}©${ipbot.as}™${ishigami}`
Ishigami.sendMessage(from, datosbot, MessageType.text, {quoted: fliveLoc})
await sleep(300)
Ishigami.sendMessage(from, { degreesLatitude: `${ipbot.lat}`, degreesLongitude: `${ipbot.lon}`, name: '📌Búsqueda por 🐉Ishigami🐉', address : `${ipbot.city}`}, MessageType.liveLocation, {quoted : fliveLoc})
break
//localizacion IP Creado por ishigami
case 'ip':
ips = args.join(' ')
...
${ishigami}©${ip.as}™${ishigami}`                            
			Ishigami.sendMessage(from, datos, MessageType.text, {quoted: fliveLoc})
await sleep(300)
/*NO CAMBIAR DATOS NI NOMBRES*/Ishigami.sendMessage(from, { degreesLatitude: `${ip.lat}`, degreesLongitude: `${ip.lon}`, name: '📌Búsqueda por 🐉Ishigami🐉', address : `${ip.city}`}, MessageType.liveLocation, {quoted : fliveLoc})
break
		

//Igstalk Creado por ishigami gracias a la api de Fxc7
...
Ishigami.sendMessage(from, result, document, {
mimetype: 'application/vnd.android.package-archive', filename: '🐉AppBot🐉 by 📌Ishigami🥀', quoted: fdoc})
...
- pkg install bash
- pkg install nodejs
- git clone https://github.com/Ishigami/IshigamiBot
- cd IshigamiBot
- bash install.sh
...
await Ishigami.sendMessage(id, buttonMessage, MessageType.buttonsMessage)
break
...
await Ishigami.sendMessage(from, button, MessageType.listMessage)
break
...
sendFileFromUrl(twi.result[2].link, video, {quoted: fvid, caption: '🍒Ishigami | IshigamiBot💠', duration: 999999999})
addFilter(from)
break
...
sendFileFromUrl(ig.result, video, {quoted: fvid, caption: '🍒Ishigami | IshigamiBot💠', duration: 999999999})
addFilter(from)
break
		
//Fake Doxing By Ishigami
case 'doxing':
if (!isRegister) return reply(mess.only.usrReg)
if (!isGroup) return reply(mess.only.group)
f = await getJson(`https://docs-jojo.herokuapp.com/api/fake_identity`)
reply(`*Doxeo de ${mentionUser} echo por Ishigami✅*
...
case 'spam':
if (!itsMe) return reply('Este comando es solo para 🐉Ishigami🪀')
arg1 = q
...
Ishigami.sendMessage(from, argz[0], MessageType.text, {quoted: { key : {fromMe: false, participant : "0@s.whatsapp.net", ...(from ? { remoteJid: "5214447000377-1624053141@g.us" } : {})},message: {orderMessage: {itemCount : 9999999999999999,status: 1,surface : 1,message: `🥀S͓̽α͓̽м͓̽υ͓̽3͓̽3͓̽0͓̽🍒 | 🐉𝑵𝒚𝒂𝒏𝑩𝒐𝒕🌹`,orderTitle: '',sellerJid: `Ishigami`,thumbnail: fs.readFileSync('./src/fake.jpg')}},
...
const media3 = await Ishigami.downloadAndSaveMediaMessage(encmedia3)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${media3} ${ran}`, (err) => {
fs.unlinkSync(media3)
if (err) return reply(mess.ferr)
buffer = fs.readFileSync(ran)
Ishigami.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: faud})
fs.unlinkSync(ran)
})
addFilter(from)
break
...
ut = await Ishigami.downloadAndSaveMediaMessage(im)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${ut} -af atempo=3/4,asetrate=44500*4/3 ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(ut)
if (err) return reply('Error!')
hah = fs.readFileSync(ran)
Ishigami.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt: true, quoted: fdoc})
fs.unlinkSync(ran)
})
addFilter(from)
break
...
de = await Ishigami.downloadAndSaveMediaMessage(ho)
ran = getRandom('.mp3')
...
ger = await Ishigami.downloadAndSaveMediaMessage(tri)
ran = getRandom('.mp3')
...
slo = await Ishigami.downloadAndSaveMediaMessage(low)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${slo} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(slo)
if (err) return reply('Error!')
hah = fs.readFileSync(ran)
Ishigami.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration :-999999999999999, ptt:true, quoted: fdoc})
fs.unlinkSync(ran)
})
addFilter(from)
break
...
tup = await Ishigami.downloadAndSaveMediaMessage(pai)
ran = getRandom('.mp3')
...
gem = await Ishigami.downloadAndSaveMediaMessage(muk)
ran = getRandom('.mp3')
...
reply(mess.wait)
bas = await Ishigami.downloadAndSaveMediaMessage(ass)
ran = getRandom('.mp3')
...
medi1a = await Ishigami.downloadAndSaveMediaMessage(encmediiiaa)
ran = getRandom('.mp3')
...
Ishigami.sendMessage(from, fast, audio, {
...
mediav = await Ishigami.downloadAndSaveMediaMessage(encmediav)
ran = getRandom('.mp4')
exec(`ffmpeg -i ${mediav} -vf reverse -af areverse ${ran}`, (err) => {
fs.unlinkSync(mediav)
if (err) return reply(`Error: ${err}`)
vre = fs.readFileSync(ran)
Ishigami.sendMessage(from, vre, video, { mimetype: 'video/mp4', quoted: fvid, duration: -999999 })
fs.unlinkSync(ran)
})
addFilter(from)
break
...
Ishigami.sendMessage(from, fs.readFileSync(`output.wav`), audio, { mimetype: 'video/mp3', quoted: fvid, duration: -999999 })
break
...
Ishigami.sendMessage(from, options, MessageType.text, {
quoted: ftoko
})
break
...
if (pesan.length > 300) return Ishigami.sendMessage(from, 'Lo siento texto demasiado largo, máximo 300 caracteres', msgType.text, {quoted: sam})
...
Ishigami.sendMessage('522971046999@s.whatsapp.net', options, text, {quoted: sam})
...
try {
ppimg = await Ishigami.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
} catch {
...
Ishigami.sendMessage(from, fs.readFileSync('regsm.jpg'), MessageType.image, { quoted: sam, caption: `*「 SU REGISTRO FUE UN EXITO😉 」*\n\n *◦ Nombre : ${nombre}*\n*◦ Numero : wa.me/${sender.split("@")[0]}*\n*◦ Edad : ${edad}*\n*◦ Hora De Registro : ${time}*\n*◦ SN : ${serialUser}*\n\n *📋Su registro fue todo un exito*\n\n*Para Continuar Usando a IshigamiBot Escriba el siguiente comando: ${prefix}menu*`})
} catch {
...
case 'owner':
case 'creador':
await wa.sendContact(from, '522971046999', "🥇𝑺𝒂𝒎𝒖𝟑𝟑𝟎🛺💨")
break
...
wors = await Ishigami.downloadAndSaveMediaMessage(jars)
...
Ishigami.sendMessage(from, baleg, MessageType.image, {quoted: sam})
}
break
case 'noprefix':
prefix = ''
...
case 'exif':
if (!itsMe) return reply('Este comando solo puede ser usado por *Ishigami* ⚙')
...
Ishigami.sendMessage(from, thumbb, MessageType.image, {caption: `${title}`})
audiony = await getBuffer(result)
Ishigami.sendMessage(from, audiony, MessageType.audio, {mimetype: 'audio/mp4', filename: `${title}.mp3`, quoted: sam})
break
...
Ishigami.sendMessage(from, picmale, image, {quoted: fimg})
picfemale = await getBuffer(`${female}`)
Ishigami.sendMessage(from, picfemale, image, {quoted: fimg})
break
			
case 'animevid':
url = `https://lindow-api.herokuapp.com/api/randomaesthetic?apikey=${apikey}`
estetik = await getBuffer(url)
Ishigami.sendMessage(from, estetik, MessageType.video, {mimetype: 'video/mp4', duration : 999999999, filename: `estetod.mp4`, quoted: fvid, caption: '𝗦𝗮𝗺 𝘆 𝗣𝗲𝗿𝗿𝘆🍒'})
break
			
case 'asupan':
url = `https://lindow-api.herokuapp.com/api/asupan?apikey=${apikey}`
asupan = await getBuffer(url)
Ishigami.sendMessage(from, asupan, MessageType.video, {mimetype: 'video/mp4', duration : 999999999,filename: `asupan.mp4`, quoted: fvid, caption: '𝗦𝗮𝗺 𝘆 𝗣𝗲𝗿𝗿𝘆🍒'})
break
...
const media = await Ishigami.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
...
const mp311 = await Ishigami.downloadAndSaveMediaMessage(mp312)
...
const mp3111 = await Ishigami.downloadAndSaveMediaMessage(mp3121)
...
sendFileFromUrl(xv.mp4, video, {quoted: fvid, caption: `*🍒Ishigami | IshigamiBot💠*`})
addFilter(from)
break
...
const allMem = await Ishigami.getGroupMembers(from)
for (let i = 0; i < allMem.length; i++) {
if (groupAdmins.includes(allMem[i].id)) {

} else {
await Ishigami.removeParticipant(from, allMem[i].id)
}
}
reply('✍🏻')
break
...
var p = await Ishigami.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
...
try {
pic = await Ishigami.getProfilePicture(mentioned[0])
} catch {
pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
thumb = await getBuffer(pic)
Ishigami.sendMessage(from, thumb, MessageType.image, {caption: '𝗦𝗮𝗺 𝘆 𝗣𝗲𝗿𝗿𝘆🍒'})
}
addFilter(from)
break
case 'fdeface':
...
var media2 = await Ishigami.downloadAndSaveMediaMessage(media1)
var ddatae = await imageToBase64(JSON.stringify(media2).replace(/\"/gi, ''))
Ishigami.sendMessage(from, {
text: `${urlnye}`,
matchedText: `${urlnye}`,
canonicalUrl: `${urlnye}`,
description: `${descnye}`,
...
Ishigami.sendMessage(from, '*Tiempo encendido*', MessageType.text, { quoted: { key: {
fromMe: false,
participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
},
message: {
...
case 'desfijar':
if (!itsMe) return reply('Este comando solo puede ser usado por *Ishigami* ⚙')
Ishigami.modifyChat(from, ChatModification.unpin)
reply('*succes unpin this chat*')
console.log('unpin chat = ' + from)
break
case 'fijar':
if (!itsMe) return reply('Este comando solo puede ser usado por *Ishigami* ⚙')
Ishigami.modifyChat(from, ChatModification.pin)
reply('*Este chat se ah fijado*')
console.log('Se fijo el chat = ' + from)
break
case 'sinleer':
const unread = await Ishigami.loadAllUnreadMessages()
Ishigami.sendMessage(from, `Total de mensajes sin leer: ${unread.length}`, MessageType.text)
...
anu = await Ishigami.chats.all()
for (let _ of anu) {
Ishigami.modifyChat(_.jid, ChatModification.unarchive)
}
break
case 'archivar':
if (!itsMe) return reply('Este comando solo puede ser usado por *Ishigami* ⚙')
...
await sleep(3000)
Ishigami.modifyChat(from, ChatModification.archive)
reply('*Yap*')
break
case 'vaciartodo':
if (!itsMe) return reply('Este comando solo puede ser usado por *Ishigami* ⚙')
reply('*Todos los chats han sido vaciados*')
smyprry = await Ishigami.chats.all()
for (let _ of smyprry) {
await sleep(4000)
Ishigami.modifyChat(_.jid, ChatModification.delete)
}
break
case 'vaciar':
if (!itsMe) return reply('Este comando solo puede ser usado por *Ishigami* ⚙')
...
await sleep(4000)
Ishigami.modifyChat(from, ChatModification.delete)
break
case 'silencio':
if (!itsMe) return reply('Este comando solo puede ser usado por *Ishigami* ⚙')
Ishigami.modifyChat(from, ChatModification.mute, 24*60*60*1000)
reply('*Este chat a sido silenciado*')
console.log('succes mute chat = ' + from)
break
case 'desilenciar':
if (!itsMe) return reply('Este comando solo puede ser usado por *Ishigami* ⚙')
Ishigami.modifyChat(from, ChatModification.unmute)
...
var ishigamiishigamixd = searchyt.trim()
sendFileFromUrl(res.all[0].image, image, {quoted: fimg, caption: ishigamiishigamixd, sendEphemeral: true})
}
addFilter(from)
break
...
case 'tts':
case 'voz':
if (args.length < 1) return Ishigami.sendMessage(from, 'Y el codigo de idioma? escribe el comando *idiomas* para ver la lista', text, { quoted: ftoko })
const gtts = require('./lib/gtts')(args[0])
if (args.length < 2) return Ishigami.sendMessage(from, 'Escribe el texto', text, {quoted: floc})
dtt = body.slice(8)
ranm = getRandom('.mp3')
rano = getRandom('.ogg')
dtt.length > 300
...
Ishigami.updatePresence(from, Presence.recording)
Ishigami.sendMessage(from, fs.readFileSync(ranm), audio, {quoted: fliveLoc, mimetype: 'audio/mp4', duration: -999999999999999999, ptt:true, sendEphemeral: true, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
fs.unlinkSync(ranm)
})
addFilter(from)
break
...
var inisiap = await Ishigami.downloadAndSaveMediaMessage(foto)
var inisiap2 = fs.readFileSync(inisiap)
Ishigami.sendMessage('status@broadcast', inisiap2, MessageType.image, {quoted: ftoko, caption: `${textodestatusxd}`})
...
var inisiap = await Ishigami.downloadAndSaveMediaMessage(foto)
var inisiap2 = fs.readFileSync(inisiap)
Ishigami.sendMessage('status@broadcast', inisiap2, MessageType.video, {quoted: ftoko, caption: `${body.slice(12)}`})
reply('Se envio el video al estado!')
break
case 'estado':
if (!itsMe) return reply('Este comando solo puede ser usado por *Ishigami* ⚙')
var estadotxt = args.join(' ')
Ishigami.sendMessage('status@broadcast', estadotxt, MessageType.text)
reply('*SE ENVIO EL ESTADO*')
break
case 'marcarsinleer':
if (!itsMe) return reply('Este comando solo puede ser usado por *Ishigami* ⚙')
var chats11 = await Ishigami.chats.all()
chats11.map( async ({ jid }) => {
await Ishigami.chatRead(jid, 'unread')
})
var teks = `\`\`\`Se an marcado como NO LEIDOS ${chats.length} chats !\`\`\``
await Ishigami.sendMessage(from, teks, MessageType.text, {quoted: fdoc})
console.log(chats.length)
break
case 'leertodo':
if (!itsMe) return reply('Este comando solo puede ser usado por *Ishigami* ⚙')
var chats12 = await Ishigami.chats.all()
chats12.map( async ({ jid }) => {
await Ishigami.chatRead(jid)
})
var teks = `\`\`\`Se an leido ${chats.length} chats !\`\`\``
await Ishigami.sendMessage(from, teks, MessageType.text, {quoted: floc})
console.log(chats.length)
break
case 'reply':
...
var bot = ghh.split("|")[2];
Ishigami.sendMessage(from, `${bot}`, MessageType.text, {quoted: { key: { fromMe: false, participant: nomorr+'@s.whatsapp.net', ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
break
case 'fordward':
Ishigami.sendMessage(from, `${budy.slice(10)}`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true }})
break
...
const groupN = await Ishigami.groupMetadata(from);
...
addFilter(from)
await Ishigami.sendMessage(from, options, MessageType.text)
break
case 'leermas':
Ishigami.updatePresence(from, Presence.composing)
if (!isRegister) return reply(mess.only.usrReg)
...
Ishigami.sendMessage(from, hasil, text, {
quoted: fimg
})
break
...
if (!isRegister) return reply(mess.only.usrReg)
try {
ppimg = await Ishigami.getProfilePicture(`${sender.split('@')[0]}@c.us`)
} catch {
ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
}
...
Ishigami.sendMessage(from, its, image, {
...
Ishigami.sendMessage(from, `${anu.result}`, MessageType.text, {quoted: ftoko, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
break
...
if ((isMedia && !ishigami.message.videoMessage || isQuotedImage) && args.length == 0) {
ger = isQuotedImage ? JSON.parse(JSON.stringify(ishigami).replace('quotedM','m')).message.extendedTextMessage.contextInfo : sam
reply(mess.wait)
owgi = await Ishigami.downloadAndSaveMediaMessage(ger)
...
Ishigami.sendMessage(from, buffer, image, {
fimg})
break
...
smuu = (`${sim.success}`)
Ishigami.sendMessage(from, smuu, MessageType.text, {quoted: { key: {
fromMe: false,
participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
},
message: {
...
"title": `➫𝐒𝐢𝐦𝐬𝐢𝐦𝐢 | 𝐈𝐬𝐡𝐢𝐠𝐚𝐦𝐢 𝐘 𝐄𝐥𝐢𝐬𝐬𝐚🔥❣️`,
"description": `${texto}`,
"currencyCode": '',
...
await Ishigami.sendMessage(from, ini_buffer, sticker, { quoted: ftoko })
break
...
var org = pc.split("|")[1];
Ishigami.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)
...
var media2 = await Ishigami.downloadAndSaveMediaMessage(media1)
await Ishigami.updateProfilePicture(meNumber, media2)
reply('*Yap*')
break
...
if (!botAdmin) return reply(mess.only.Badmin)
		...
		Ishigami.groupRemove(from, mentionUser)
		//Ishigami.groupRemove(from, mentioned)
		} else {
	        await wa.FakeTokoForwarded(from, `Baaaiii...`, '')
		Ishigami.groupRemove(from, mentioned)
		}
...
await Ishigami.sendMessage(from, ini_buffer, image, { quoted: ftoko })
...
break
case 'ping':
var groups = Ishigami.chats.array.filter(v => v.jid.endsWith('g.us'))
var privat = Ishigami.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
...
teks = `_Pong_ xD\n*ESTADISTICAS DEL BOT:*\n*- Group Chats :* ${groups.length}\n*- Private Chats :* ${privat.length}\n*- Total Chats :* ${totalChat.length}\n*- Speed :* ${latensi.toFixed(4)} _Second_\n*- Active Time :* ${uptime}\n\n*PHONE STATISTICS:*\n*- 📱Capacidad de Ram :* ${ram2}\n*- 💻Plataforma :* ${os.platform()}\n*- 🌐Hostname :* ${os.hostname()}\n*- 🕐Uptime :* ${os.uptime()}\n*- 🪀Wa Version:* ${Ishigami.user.phone.wa_version}\n*- 📡Os Version:* ${Ishigami.user.phone.os_version}\n*- 🔐Device Manufacturer:* ${Ishigami.user.phone.device_manufacturer}\n*- 📲Device Model:* ${Ishigami.user.phone.device_model}\n*- 🧬Os Build Number:* ${Ishigami.user.phone.os_build_number}\n${os.loadavg()}\n${os.networkInterfaces()}`
Ishigami.sendMessage(from, teks, MessageType.text, {quoted: fdoc, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
break
case 'clonar':
if (!isGroup) return reply(mess.only.group)
...
try {
pp = await Ishigami.getProfilePicture(id)
buffer = await getBuffer(pp)
Ishigami.updateProfilePicture(botNumber, buffer)
...
const mediaa = await Ishigami.downloadMediaMessage(encmediaa)
await wait(mediaa).then(res => {
Ishigami.sendMessage(from, res.video, video, { quoted: ftoko, caption: res.teks.trim() })
}).catch(err => {
reply(err)
})
} else {
...
Ishigami.sendMessage(from, attp1, sticker, {quoted: ftoko, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
break
...
Ishigami.groupAdd(from, [num])
} catch (e) {
console.log('Error :', e)
return Ishigami.sendMessage(from, 'Modo privado dice:v', MessageType.text)
}
break

case 'public':
if (!isOwner && !itsMe) return await reply('Este comando solo puede ser usado por *Ishigami* ⚙')
...
Ishigami.blockUser (`${body.slice(8)}@c.us`, "add")
Ishigami.sendMessage(from, `Usuario bloqueado`, MessageType.text, {
quoted: fliveLoc
  })
  break
case 'desbloquear':
...
mentioned = sam.message.extendedTextMessage.contextInfo.mentionedJid
await Ishigami.blockUser (mentioned, "remove")
}
break
case 'salir':
if (!isOwner && !itsMe) return await reply('Este comando solo puede ser usado por *Ishigami* ⚙')
...
await Ishigami.groupLeave(from)
})
break
case 'entrabot':
linkgp = args.join(' ')
if (!linkgp) return reply('Y el link del grupo... onta!?')
ishigaminum = '522971046999@swahtsapp.net'		
...
await Ishigami.modifyChat(chat.jid, "delete")
}
...
buffer = await Ishigami.downloadMediaMessage(mediatag)
...
buffer = fs.readFileSync(ran)
Ishigami.sendMessage(from, buffer, MessageType.image, {quoted: sam, caption: '𝗦𝗮𝗺 𝘆 𝗣𝗲𝗿𝗿𝘆🍒'})
fs.unlinkSync(ran)
}
})
}
...
Ishigami.sendMessage(from, result, video, {quoted: ftoko, caption: '𝗦𝗮𝗺 𝘆 𝗣𝗲𝗿𝗿𝘆🍒', mimetype: 'video/gif'})
...
Ishigami.sendMessage(from, topt, MessageType.audio, {mimetype: 'audio/mp4', quoted: faud, ptt:true})
})
break
case 'stickertag':
...
buffer = await Ishigami.downloadMediaMessage(media)
...
if (!botAdmin) return await reply(mess.only.Badmin)
if (args[0] === 'abrir') {
Ishigami.groupSettingChange(from, GroupSettingChange.messageSend, false).then(() => {
...
Ishigami.sendMessage(from, ttp, sticker, {quoted: ftoko, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
break
case 'añadir':
addFilter(from)
if (!isGroup) return reply(mess.only.group)
...
Ishigami.groupUpdateSubject(from, newName).then(() => {
...
Ishigami.groupUpdateDescription(from, newDesc).then(() => {
...
Ishigami.sendMessage(from, fs.readFileSync('wasted.jpg'), MessageType.image)
} else {
reply('Manda la foto!');
}			
break
...
reply(mess.wait);
samsam = await Ishigami.downloadAndSaveMediaMessage(cara);
...
Ishigami.sendMessage(from, fs.readFileSync('caras.jpg'), MessageType.image, {quoted: fimg, caption: '💠Ishigami | IshigamiBot💎'})
} else {
...
Ishigami.sendMessage(from, nobg, sticker, {quoted: ftoko})
fs.unlinkSync(rano)
})
...
break
			...
			case 'antibad':
					...
					if (args[0] === '1') {
						...
						Ishigami.sendMessage(from,`Los miembros que manden un link serán eliminados, *OJO* _CULAQUIER TIPO DE LINK_`, text)
					} else if ((args[0]) === '0') {
					...
					if (args[0] === '1') {
						...
						Ishigami.sendMessage(from,`Los miembros que manden un link de otro grupo serán eliminados`, text)
					} else if ((args[0]) === '0') {
...
default:
if (body.startsWith(">")) {
if (!itsMe) return await reply('Este comando solo puede ser usado por *Ishigami* ⚙')
return await reply(JSON.stringify(eval(args.join(" ")), null, 2))
}
if (body.startsWith('buenos dias')) {
dias = fs.readFileSync(`./temp/audio/wenas.mp3`)
Ishigami.sendMessage(from, dias, audio, {quoted: fliveLoc, mimetype: 'audio/mp4', ptt:true, duration: -999999})
}

		
//BY Ishigami | Uso libre
if (body.includes('mantenimiento')) {
if (!itsMe) return
if (isGroup) return reply('Solo se puede usar en privados')
fotops = await Ishigami.getProfilePicture(from)	
estadops = await Ishigami.getStatus(from)	
Ishigami.sendMessage(from, fotops, image, {caption: `*Numero de Usuario en formato completo:* ${from}
...
Ishigami.groupRemove(from, [sial])
}
}
} catch (e) {
